...
sendAuthKeyBtn.addEventListener("click", () => {
    ...
    // setInertval(함수, 지연시간(ms))
    // - 지연시가(ms) 만큼 시간이 지날 때 마다 함수 수행

    // clearInterval(interval이 저장된 변수)
    // - 매개변수로 전달받은 interval을 멈춤

    // 인증 시간 출력(1초 마다 동작)
    authTimer = setInterval( () => { // 1초 지연시간
...
memberNickname.addEventListener("input", e => {
    ...
    if( !regExp.test(inputNickname) ) { // 일치하지 않는 경우
