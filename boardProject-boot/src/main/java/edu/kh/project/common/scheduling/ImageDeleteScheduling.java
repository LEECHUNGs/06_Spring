...
public class ImageDeleteScheduling {
	...
	// @Scheduled(fixedDelay = 5000) // 이전 작업이 시작된 후 5초 후
	// @Scheduled(fixedRate = 5000) // 이전 작업이 끝난 후 5초 후 수행
	...
	@Scheduled(cron = "0 0 0 1 * *") // 시계 초 단위가 0, 30인 경우 수행 (테스트용)
	public void scheduling() {
		...
		// 1. 서버 파일 목록 조회하기
		File boardFoler = new File(boardFolderPath);
		File memberFolder= new File(profileFolderPath);
		
		// 참조하는 폴더에 존재하는 파일 목록 얻어오기
		File[] boardArr = boardFoler.listFiles();
		File[] memberArr = memberFolder.listFiles();
	
		// 두 배열을 하나로 합침 (for문 한번만 사용하기 위해..)
		// imageArr 라는 빈 배열을 boardArr와 memberArr 길이 만큼의 크기로 만듬
		File[] imageArr = new File[boardArr.length + memberArr.length];
		
		// 배열 내용 복사 (깊은 복사)
		// System.ArrayCopy(복사할 배열, 몇번 인덱스부터 복사할지, 
		//                   새로운 배열, 새로운배열의 몇번부터 넣을지 인덱스번호, 복사를 어디까지 할지)
		...
		// 3. 서버, DB 이미지 파일명을 비교해
		// 서버에만 있고, DB 에 없는 파일을 서버에서 삭제
		
		if( !serverImageList.isEmpty() ) { // 서버에 이미지가 있을 때
			
			for(File serverImage : serverImageList) {
				// File.getName() : 서버 파일의 이름
