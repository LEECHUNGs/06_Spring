...
function getTotalCount() {
    ...
    .then( reault => {
        ...
        // #totalCount 요소의 내용을 result 로 변경
        totalCount.innerText = reault;
    });
}
...
const selectTodoList = () => {
    ...
    .then(resp => resp.text()) // 응답결과를 text로 반환
    .then(result => {
        //console.log(result);
        //console.log(typeof result); // 객체가 아닌 문자열 형태

        // 문자열은 가공을 할 수 있으나 어렵다
...
changeComplete.addEventListener("click", () => {
    ...
    .then(result => {
        ...
        if(result > 0) {
            ...
            // update된 DB 데이터를 다시 조회해서 화면에 출력시
            // -> 서버 부하가 큼
...
// 상세 조회 에서 수정 버튼(#updateView) 클릭 시
...
// 수정 레이어에서 취소 버튼(#updataCancel)이 클릭되었을 때
